    ...
    describe("GIVEN alice has 23500 TP 0", function () {
      ...
      describe("WHEN alice swaps 23500(all balance) TP 0 for 100 TC", function () {
        ...
        it("THEN a TPSwappedForTC event is emitted", async function () {
          ...
          await expect(tx)
            .to.emit(mocImpl, "TPSwappedForTC")
            .withArgs(TP_0, operator, alice, pEth(23500), pEth(100), pEth(100 * 0.01), 0, 0, 0);
        });
      ...
      describe("WHEN alice swaps 2350(10% of balance) TP 0 for 10 TC to bob", function () {
        ...
        it("THEN a TPSwappedForTC event is emitted", async function () {
          ...
          await expect(tx)
            .to.emit(mocImpl, "TPSwappedForTC")
            .withArgs(TP_0, operator, bob, pEth(2350), pEth(10), pEth(10 * 0.01), 0, 0, 0);
        });
      });
      describe("WHEN alice tries to swap 23500 TP 0 for 100 TC via vendor without sending the AC for the markup", function () {
        it("THEN tx reverts because AC received is below the minimum required", async function () {
          ...
          await expect(
            mocFunctions.swapTPforTC({ i: TP_0, from: alice, qTP: 23500, qACmax: 1, vendor }),
          ).to.be.revertedWithCustomError(mocImpl, ERRORS.INSUFFICIENT_QAC_SENT);
        });
      });
      describe("WHEN alice swaps 23500 TP 0 for 100 TC via vendor", function () {
        ...
        beforeEach(async function () {
          ...
          tx = await mocFunctions.swapTPforTC({ i: TP_0, from: alice, qTP: 23500, vendor });
        });
        ...
        it("THEN a TPSwappedForTC event is emitted", async function () {
          ...
          await expect(tx)
            .to.emit(mocImpl, "TPSwappedForTC")
            .withArgs(TP_0, operator, alice, pEth(23500), pEth(100), pEth(100 * 0.01), 0, pEth(100 * 0.1), 0);
        });
      });
      describe("WHEN alice swaps 23500 TP 0 for 100 TC to bob via vendor", function () {
        let tx: ContractTransaction;
        beforeEach(async function () {
          tx = await mocFunctions.swapTPforTCto({ i: TP_0, from: alice, to: bob, qTP: 23500, vendor });
        });
        it("THEN a TPSwappedForTC event is emitted", async function () {
          // iFrom: 0
          // iTo: 1
          ...
          await expect(tx)
            .to.emit(mocImpl, "TPSwappedForTC")
            .withArgs(TP_0, operator, bob, pEth(23500), pEth(100), pEth(100 * 0.01), 0, pEth(100 * 0.1), 0);
        });
      });
        ...
        */
        ...
        describe("WHEN alice swaps 4700 TP for 9.933 TC", function () {
          ...
          it("THEN a TPSwappedForTC event is emitted", async function () {
            ...
            await expect(tx)
              .to.emit(mocImpl, "TPSwappedForTC")
              .withArgs(TP_0, operator, alice, pEth(4700), pEth("9.933774834437086092"), pEth(10 * 0.01), 0, 0, 0);
          });
        });
      });
        ...
        */
        ...
        describe("WHEN alice swaps 1000 TP for 10.471 TC", function () {
          ...
          it("THEN a TPSwappedForTC event is emitted", async function () {
            ...
            await expect(tx)
              .to.emit(mocImpl, "TPSwappedForTC")
              .withArgs(TP_0, operator, alice, pEth(1000), pEth("10.471204188481675392"), pEth(10 * 0.01), 0, 0, 0);
          });
        });
      });
      ...
      describe("AND alice has FeeToken to pay fees", function () {
        ...
        describe("WHEN alice swaps 23500 TP 0 for 100 TC", function () {
          ...
          it("THEN Fee Token is used as fee payment method", async function () {
            ...
            await expect(tx)
              .to.emit(mocImpl, "TPSwappedForTC")
              .withArgs(TP_0, operator, alice, pEth(23500), pEth(100), 0, pEth(0.5), 0, 0);
          });
        });
        describe("WHEN alice swaps 23500 TP 0 for 100 TC", function () {
          beforeEach(async function () {
          ...
          it("THEN Fee Token is used as fee payment method", async function () {
            ...
            await expect(tx)
              .to.emit(mocImpl, "TPSwappedForTC")
              .withArgs(TP_0, operator, bob, pEth(23500), pEth(100), 0, pEth(0.5), 0, 0);
          });
        });
      });
    });
